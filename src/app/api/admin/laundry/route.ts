 ...
 *      | { aksi: "rak", kode, lokasi?, aktif?, urutan? } — daftar rak yang disarankan ke terminal
 */
...
export const GET = tangani(async (req) => {
  ...
  const [aktif, tunggakan, tarif, rak, selesai] = await Promise.all([
    ...
    q(`SELECT kode, lokasi, aktif, urutan FROM rak_laundry ORDER BY urutan, kode`),
...
export const POST = tangani(async (req) => {
  ...
  const b = await bacaBody(req, v.obj({
    ...
    lokasi: v.str({ max: 80 }).opsional(), urutan: v.int({ min: 0, max: 999 }).opsional(),
  }));
  if (b.aksi === "status") {
  ...
  if (b.aksi === "rak") {
    if (!b.kode) throw new HttpError(400, "VALIDASI", "kode rak wajib");
    return ok({ rak: await skalar<string>("rak_laundry_simpan", [b.kode, b.lokasi ?? null, b.aktif ?? null, b.urutan ?? null, aktor(p)]) });
  }
