...
export const GET = tangani(async (req) => {
  ...
  const f = bacaQuery(req, v.obj({
    // 120, bukan 60: objek berformat 'staf:<email>' dan email sekolah yang
    // panjang membuat penyaringan dari layar audit ditolak sebelum sampai ke
    // kueri — padahal nilainya berasal dari baris yang tampil di layar itu.
    objek: v.str({ max: 120 }).opsional(),
    aktor: v.str({ max: 80 }).opsional(),
    aksi: v.str({ max: 40 }).opsional(),
    limit: v.int({ min: 1, max: 1000 }).default(200),
  }));
  const rows = await q(
